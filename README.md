...
- 🔊 **Per-Track Sampler Engine** — Independent `SimplerProcessor` instances per track with waveform display
...
├── SimplerProcessor.h        # Per-track sampler DSP engine
...
```bash
sudo apt install cmake build-essential libasound2-dev libfreetype6-dev \
...
```bash
git clone https://github.com/YOUR_USERNAME/LiBeDAW.git
cd LiBeDAW
